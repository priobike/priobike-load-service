...
class GetLoadResponseView(View):
    def get(self, request):
        """
        Get the load response.
        """
        status = Status.objects.order_by('-time').first()
        if not status:
            return JsonResponse({'warning': False}) # Default response
        return JsonResponse({
            'warning': status.warning,
            'response_text': status.response_text,
        })
